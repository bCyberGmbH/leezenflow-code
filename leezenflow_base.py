...
class LeezenflowBase(object):
    ...
    def process(self):
        ...
        if self.args.receiver == 0:
            ...
            t2 = threading.Thread(
                target=self.receiver(mode="mqtt"), args=(None, run_event)
            )
        ...
        if self.args.test == -1:
            ...
            t2 = threading.Thread(
                target=Simulation.phase_switch_simulation_4_phases, args=(5, run_event)
            )
        elif self.args.test == 1:
            t2 = threading.Thread(target=Simulation.mqtt_log, args=(None, run_event))
        elif self.args.test == 2:
            t2 = threading.Thread(
                target=Simulation.log_simulation, args=(None, run_event)
            )
        elif self.args.test == 3:
            t2 = threading.Thread(
                target=Simulation.phase_switch_simulation, args=(5, run_event)
            )
        elif self.args.test == 4:
            t2 = threading.Thread(
                target=Simulation.phase_switch_simulation, args=(30, run_event)
            )
        elif self.args.test == 5:
            t2 = threading.Thread(
                target=Simulation.phase_update_simulation, args=("red", run_event)
            )
        elif self.args.test == 6:
            t2 = threading.Thread(
                target=Simulation.phase_fast_simulation, args=("green", run_event)
            )
        elif self.args.test == 7:
            t2 = threading.Thread(
                target=Simulation.stale_prediction, args=(None, run_event)
            )
        ...
        try:
            ...
            print(
                "Cannot run without either test dataset or receiver. Use flag --test 0 to run with test dataset 0."
            )

...
class Interpreter:
    ...
    def interpret_message(self, message):
        try:
            ...
            if root.tag == "SPATEM":
                time_stamp = int(
                    root.find("spat/intersections/IntersectionState/timeStamp").text
                )
                moy = int(root.find("spat/intersections/IntersectionState/moy").text)
                likely_time = int(
                    root.find(
                        "spat/intersections/IntersectionState/states/MovementState/state-time-speed/MovementEvent/timing/likelyTime"
                    ).text
                )
                phase = root.find(
                    "spat/intersections/IntersectionState/states/MovementState/state-time-speed/MovementEvent/eventState"
                )[0].tag
                ...
                if (
                    likely_time / 10 <= (moy % 60) * 60
                ):  # If true, likely_time corresponds to next hour
                    ...
                    remaining_phase_seconds = (
                        predicted_seconds_of_hour - current_seconds_of_hour
                    )
